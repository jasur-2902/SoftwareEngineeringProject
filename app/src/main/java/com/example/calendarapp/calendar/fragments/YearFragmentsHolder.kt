...
import com.example.calendarapp.calendar.adapters.MyYearPagerAdapter
...
class YearFragmentsHolder : MyFragmentHolder() {
            ...
            addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                ...
                override fun onPageSelected(position: Int) {
                    ...
                    if (isGoToTodayVisible != shouldGoToTodayBeVisible) {
                        (activity as? CalendarActivity)?.toggleGoToTodayVisibility(shouldGoToTodayBeVisible)
                        isGoToTodayVisible = shouldGoToTodayBeVisible
                    }

                    if (position < years.size) {
                        (activity as? CalendarActivity)?.updateActionBarTitle("${getString(R.string.app_launcher_name)} - ${years[position]}")
                    }
                }
            })
    ...
    override fun updateActionBarTitle() {
        (activity as? CalendarActivity)?.updateActionBarTitle("${getString(R.string.app_launcher_name)} - $currentYear")
    }
