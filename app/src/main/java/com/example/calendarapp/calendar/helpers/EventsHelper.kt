...
class EventsHelper(val context: Context) {
    ...
    fun getEventTypes(activity: AppCompatActivity, showWritableOnly: Boolean, callback: (notes: ArrayList<EventType>) -> Unit) {
    ...
    fun insertOrUpdateEventType(activity: AppCompatActivity, eventType: EventType, callback: ((newEventTypeId: Long) -> Unit)? = null) {
        ensureBackgroundThread {
    ...
    fun getEventsWithSearchQuery(text: String, activity: AppCompatActivity, callback: (searchedText: String, events: List<Event>) -> Unit) {
        ensureBackgroundThread {
